...
public class WithdrawMoneyScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MW.text = "Minimum withdrawal is BDT. " + PlayerPrefs.GetInt("MW", 50).ToString(); 
        ...
        if (!Done)
            NetAmount = 0;
        NetAmountText.text = "Net Balance  BDT. " + NetAmount.ToString();
    ...
    public void callAPI()
    {
        ...
        {
            if(UPI.isOn)
            ErrorText.GetComponent<Text>().text = "Please Enter bKash Number";
            else if(Paytm.isOn)
            ErrorText.GetComponent<Text>().text = "Please Enter Nogot Number";
            else if(Bank.isOn)
        ...
        if ((withdrawlAmount.GetComponent<InputField>().text) != "" && (accountNumber.GetComponent<InputField>().text) != "")
        {
            ...
            if (done)
            {
                ...
                if ((amountWithdrawl <= NetAmount) && (amountWithdrawl >= PlayerPrefs.GetInt("MW", 50)))
                {
                    ...
                    {
                        type = "paytm";
                        if (accountNumber.GetComponent<InputField>().text.Length != 11)
                        {
                            saveBtn.SetActive(true);
                            GameManager.Instance.dialogNew.dialogTxt.GetComponent<Text>().text = "Please fill all the details.";
                            GameManager.Instance.objectGame.SetActive(true);
    ...
    IEnumerator WaitForRequest(UnityWebRequest www)
    {
        ...
        if (www.error == null)
        {
            ...
            {
                NetAmountText.text = "Net Balance  BDT."+ jsonNode["wincoin"];
    ...
    public void HandleDropdownInputData(int val)
    {
        ...
        if (val == 0)
        {
            ...
            accountNumber.GetComponent<InputField>().placeholder.GetComponent<Text>().text = "Enter bKash Number";
        ...
        else if (val == 1)
        {
            ...
            Bank.isOn = false;
            accountNumber.GetComponent<InputField>().placeholder.GetComponent<Text>().text = "Enter Nogot Number";
            bankName.SetActive(false); ifscCode.SetActive(false);
